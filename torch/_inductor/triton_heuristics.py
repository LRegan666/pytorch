...
class CachingAutotuner(KernelInterface):
    ...
    def __init__(
        ...
        self.gpgpu_device = get_interface_for_device(self.triton_meta["device_type"])
    ...
    def precompile(self, warm_cache_only_with_cc=None):
        with self.lock:
            ...
            device_prop = self.gpgpu_device.Worker.get_device_properties(
    ...
    def _precompile_config(self, cfg: Config, warm_cache_only_with_cc: Optional[int]):
        ...
        if warm_cache_only_with_cc:
            ...
            device_type = (
                self.triton_meta["device_type"] if torch.version.hip is None else "cuda"
            )
            device_id = compile_meta["device"]
            device = torch.device(device_type, device_id)
            cc = self.gpgpu_device.get_compute_capability(device)
        ...
        with self.gpgpu_device.device(compile_meta["device"]):  # type: ignore[attr-defined]
            # need to initialize context
            self.gpgpu_device.synchronize(self.gpgpu_device.current_device())
        ...
        scope = {
            ...
            "bin": binary,
            "torch": torch,
            "set_device": self.gpgpu_device.set_device,
            "current_device": self.gpgpu_device.current_device,
        }
    ...
    def bench(self, launcher, *args, grid, **kwargs):
        ...
        stream = self.gpgpu_device.get_raw_stream(  # type: ignore[call-arg]
            self.gpgpu_device.current_device()
        )
